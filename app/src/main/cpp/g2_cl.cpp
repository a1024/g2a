...
G2_R_RR(logic_gerater){IDX; ASSIGN_R(xr[idx]>yr[idx]);}
G2_R_RC(logic_gerater){IDX; ASSIGN_R(xr[idx]>yr[idx]);}
G2_R_RQ(logic_gerater){IDX; ASSIGN_R(xr[idx]>yr[idx]);}
G2_R_CR(logic_gerater){IDX; ASSIGN_R(xr[idx]>yr[idx]);}
G2_R_CC(logic_gerater){IDX; ASSIGN_R(xr[idx]>yr[idx]);}
G2_R_CQ(logic_gerater){IDX; ASSIGN_R(xr[idx]>yr[idx]);}
...
G2_R_RR(logic_gerater_equal){IDX; ASSIGN_R(xr[idx]>=yr[idx]);}
G2_R_RC(logic_gerater_equal){IDX; ASSIGN_R(xr[idx]>=yr[idx]);}
G2_R_RQ(logic_gerater_equal){IDX; ASSIGN_R(xr[idx]>=yr[idx]);}
G2_R_CR(logic_gerater_equal){IDX; ASSIGN_R(xr[idx]>=yr[idx]);}
G2_R_CC(logic_gerater_equal){IDX; ASSIGN_R(xr[idx]>=yr[idx]);}
G2_R_CQ(logic_gerater_equal){IDX; ASSIGN_R(xr[idx]>=yr[idx]);}
...
//auxiliary constants
__global extern float *constants;
enum Constants
{
	C_PI, C_2PI, SQRT_2PI, C_SQRT2, C_LN2, C_INV_LN10, C_LN_PHI, C_INV_SQRT5,
};
...
int		iscastable2int(float x);
int		f2i(float x);
bool	_1d_int_in_range(float a, float b);
bool	_1d_zero_in_range(float a, float b);
bool	istrue_c(float2 a);
bool	istrue_q(float4 a);
float	abs_c(float2 a);
float	abs_q(float4 a);
bool	equal_rc(float a, float2 b);
bool	equal_rq(float a, float4 b);
bool	equal_cr(float2 a, float b);
bool	equal_cc(float2 a, float2 b);
bool	equal_cq(float2 a, float4 b);
bool	equal_qr(float4 a, float b);
bool	equal_qc(float4 a, float2 b);
bool	equal_qq(float4 a, float4 b);
float2	floor_c(float2 a);
float4	floor_q(float4 a);
float2	sq_c(float2 a);
float4	sq_q(float4 a);
float2	sqrt_c(float2 a);
float4	sqrt_q(float4 a);
...
DISC_QQ_I(modulo){IDX; disc[idx]=false;}
	...
	static const char program_src_pt3[]=R"CLSRC(
//kernels pt3
G2_R_R(sgn){IDX; float a=xr[idx]; ASSIGN_R((a>0)-(a<0));}
G2_C_C(sgn)
{
...
DISC_Q_I(acoth){IDX; disc[idx]=false;}//TODO
	...
	static const char program_src_pt4[]=R"CLSRC(
//kernels pt4
G2_R_R(exp){IDX; ASSIGN_R(exp(xr[idx]));}
G2_C_C(exp){IDX; float2 ret=exp_c(VEC2(x)); RET_C;}
G2_Q_Q(exp){IDX; float4 ret=exp_q(VEC4(x)); RET_Q;}
...
//__kernel void initialize_const(__global const int *size, __global float *buffer, __global const float value)//use clEnqueueFillBuffer
//{
//	IDX;
//	buffer[idx]=value;
//}
